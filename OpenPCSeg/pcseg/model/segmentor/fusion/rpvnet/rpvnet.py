...
from OpenPCSeg.package.torchsparse.torchsparse import PointTensor
from OpenPCSeg.package.torchsparse.torchsparse.tensor import SparseTensor

from OpenPCSeg.pcseg.model.segmentor.base_segmentors import BaseSegmentor
from .utils import initial_voxelize, point_to_voxel, voxel_to_point

from OpenPCSeg.pcseg.loss import Losses


import OpenPCSeg.package.range_lib.range_utils.nn.functional as rnf
...
def resample_grid_stacked(predictions, pxpy, grid_sample_mode='bilinear'):
    ...
    for cnt, one_batch in enumerate(predictions):
        #bs_mask = (pxpy[cnt][:, 0] == cnt)
        one_batch = one_batch.unsqueeze(0)
        #print('one batch:', one_batch.size())
        #one_pxpy = pxpy[cnt][bs_mask][:, 1:].unsqueeze(0).unsqueeze(0)
        one_pxpy = pxpy[cnt].unsqueeze(0).unsqueeze(0)
        
        #print('one_pxpy shape:', one_pxpy.shape)
